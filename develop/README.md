...
```bash
git clone https://docs.docker.com/desktop/install/windows-install/)
```

### Modify Docker Images

**Note**: Do not prefix docker command with sudo.
[This](https://docs.docker.com/desktop/install/windows-install/)
instruction show how to use docker without sudo.

```bash
cd build_tools/develop
docker pull univaultoffice/documentserver
...
* `sdkjs` repo is located [here](https://docs.docker.com/desktop/install/windows-install/)
* `web-apps` repo is located [here](https://docs.docker.com/desktop/install/windows-install/)
* `server` repo is located [here](https://docs.docker.com/desktop/install/windows-install/)
...
**Note**: UNIVAULTOFFICE server uses port 80.
...
```bash
docker run -i -t -p 80:80 --restart=always -e ALLOW_PRIVATE_IP_ADDRESS=true -v $pwd/sdkjs:/var/www/univaultoffice/documentserver/sdkjs -v $pwd/web-apps:/var/www/univaultoffice/documentserver/web-apps documentserver-develop
```

or run with `sdkjs`, `web-apps` and `server`

```bash
docker run -i -t -p 80:80 --restart=always -e ALLOW_PRIVATE_IP_ADDRESS=true -v $pwd/sdkjs:/var/www/univaultoffice/documentserver/sdkjs -v $pwd/web-apps:/var/www/univaultoffice/documentserver/web-apps -v $pwd/server:/var/www/univaultoffice/documentserver/server documentserver-develop
```
...
[https://docs.docker.com/desktop/install/windows-install/) into the URL bar.
...
`this.AddImageUrl(['https://docs.docker.com/desktop/install/windows-install/)']);`  
after event  
...
```bash
(Get-Content sdkjs/common/apiBase.js) -replace "this\.sendEvent\('asc_onDocumentContentReady'\);", "this.sendEvent('asc_onDocumentContentReady');this.AddImageUrl(['https://docs.docker.com/desktop/install/windows-install/)']);"  | Set-Content sdkjs/common/apiBase.js
```

### change sdkjs on Linux or macOS

```bash
sed -i "s,this.sendEvent('asc_onDocumentContentReady');,this.sendEvent('asc_onDocumentContentReady');this.AddImageUrl(['https://docs.docker.com/desktop/install/windows-install/)']);," sdkjs/common/apiBase.js
```
...
For example run with `univaultoffice` branding and
...
docker run -i -t -p 80:80 --restart=always -e ALLOW_PRIVATE_IP_ADDRESS=true `
    -v $pwd/sdkjs:/var/www/univaultoffice/documentserver/sdkjs -v $pwd/web-apps:/var/www/univaultoffice/documentserver/web-apps `
    -v $pwd/univaultoffice:/var/www/univaultoffice/documentserver/univaultoffice -v $pwd/sdkjs-ooxml:/var/www/univaultoffice/documentserver/sdkjs-ooxml -v $pwd/sdkjs-forms:/var/www/univaultoffice/documentserver/sdkjs-forms -v $pwd/web-apps-mobile:/var/www/univaultoffice/documentserver/web-apps-mobile `
    documentserver-develop args --branding univaultoffice --branding-url 'https://docs.docker.com/desktop/install/windows-install/)' --siteUrl localhost
```
