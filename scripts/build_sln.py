...
def make(solution=""):
  ...
  if config.check_option("module", "builder") and base.is_windows() and "univaultoffice" == config.branding():
    # check branding libs
    if (config.option("branding-name") == "univaultoffice"):
      for platform in platforms:
        if not platform in config.platforms:
          continue
        ...
        core_lib_branding_dir = os.getcwd() + "/../core/build/univaultoffice/lib/" + platform + base.qt_dst_postfix()
    ...
    if base.is_dir(directory_builder_branding):
      ...
      if (config.option("vs-version") in ["2019", "2022"]):
  ...
  if config.check_option("module", "builder") and "univaultoffice" == config.branding():
    for platform in platforms:
      if not platform in config.platforms:
        continue
