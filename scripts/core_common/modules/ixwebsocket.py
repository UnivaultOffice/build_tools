  ...
  if (-1 != config.option("platform").find("android")):
   ...
   build_arch("android", "x86", param_android("x86", "16"))
    ...
    if (config.option("vs-version") == "2019"):
      vsVersion = "16 2019"
    elif (config.option("vs-version") == "2022"):
      vsVersion = "17 2022"
    ...
    if (-1 != config.option("platform").find("win_64")):
      if config.option("vs-version") == "2022":
        win64_gen = ["-G", "Visual Studio " + vsVersion, "-A", "x64"]
      else:
        win64_gen = ["-G", "Visual Studio " + vsVersion + " Win64"]
      build_arch("windows", "win_64", win64_gen)
      build_arch("windows_debug", "win_64", win64_gen, True)
