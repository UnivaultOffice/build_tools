...
def _download_resume(url, dst, label=""):
  if host_platform() == "windows":
    ps_cmd = (
      ...
      "if($len -ge 0 -and $pos -ge $len){"
      "  Write-Host ('[cache] part ' + $label + ' already complete: ' + $lenMB + ' MB');"
      "  exit 0;"
      "};"
      "Write-Host ('[cache] part ' + $label + ' total: ' + $lenMB + ' MB');"
      ...
      "$lastReport=[DateTime]::UtcNow; $lastBytes=$pos; $lastTime=$lastReport;"
      "$total=$pos;"
      ...
      "    $pct=[Math]::Round(($total/$len)*100,1);"
      "    $deltaBytes=$total-$lastBytes;"
      "    $deltaSec=($now-$lastTime).TotalSeconds;"
      "    if($deltaSec -le 0){$deltaSec=1};"
      "    $speedMB=[Math]::Round(($deltaBytes/1MB)/$deltaSec,1);"
      "    $remainBytes=$len-$total;"
      "    $etaSec=0;"
      "    if($speedMB -gt 0){$etaSec=[Math]::Round(($remainBytes/1MB)/$speedMB)};"
      "    $etaStr=([TimeSpan]::FromSeconds($etaSec)).ToString();"
      "    Write-Host -NoNewline ('`r[cache] part ' + $label + ' ' + $totalMB + '/' + $lenMB + ' MB (' + $pct + '%) ' + $speedMB + ' MB/s ETA ' + $etaStr + '     ');"
      "    $lastReport=$now; $lastBytes=$total; $lastTime=$now;"
      "  }"
      "};"
      "$totalMB=[Math]::Round($total/1MB,1);"
      "$pct=[Math]::Round(($total/$len)*100,1);"
      "Write-Host -NoNewline ('`r[cache] part ' + $label + ' ' + $totalMB + '/' + $lenMB + ' MB (' + $pct + '%) done                      ');"
      "Write-Host '';"
...
def _parse_cmake_version(text):
  match = re.search(r"cmake version ([0-9]+)\\.([0-9]+)\\.([0-9]+)", text)
  if not match:
    return ()
  return (int(match.group(1)), int(match.group(2)), int(match.group(3)))

def _get_cmake_version(cmake_exe):
  if not is_file(cmake_exe):
    return ()
  cmd = "\"" + cmake_exe + "\" --version"
  out = run_command(cmd)
  text = out["stdout"] if out["stdout"] else out["stderr"]
  return _parse_cmake_version(text)

def _find_cmake_bin_dir():
  candidates = []
  program_dirs = []
  if get_env("ProgramFiles"):
    program_dirs.append(get_env("ProgramFiles"))
  ...
  for root in program_dirs:
    for version in versions:
      for edition in editions:
        path = os.path.join(root, "Microsoft Visual Studio", version, edition,
                            "Common7", "IDE", "CommonExtensions", "Microsoft", "CMake", "CMake", "bin")
        if is_file(path + "\\cmake.exe"):
          candidates.append(path)

  cmake_home = get_env("CMAKE_HOME")
  if cmake_home and is_file(cmake_home + "\\bin\\cmake.exe"):
    candidates.append(cmake_home + "\\bin")

  if get_env("ProgramFiles") and is_file(get_env("ProgramFiles") + "\\CMake\\bin\\cmake.exe"):
    candidates.append(get_env("ProgramFiles") + "\\CMake\\bin")

  if get_env("ProgramFiles(x86)") and is_file(get_env("ProgramFiles(x86)") + "\\CMake\\bin\\cmake.exe"):
    candidates.append(get_env("ProgramFiles(x86)") + "\\CMake\\bin")

  tools_cmake_dir = get_script_dir() + "/../tools/win/cmake"
  if is_dir(tools_cmake_dir):
    for item in glob.glob(tools_cmake_dir + "/cmake-*/bin/cmake.exe"):
      candidates.append(os.path.dirname(item))

  # pick the newest < 4.0.0 if available
  best = ""
  best_ver = ()
  for path in candidates:
    ver = _get_cmake_version(path + "\\cmake.exe")
    if not ver:
      continue
    if ver < (4, 0, 0) and (best_ver == () or ver > best_ver):
      best = path
      best_ver = ver
  ...
  # fallback to the newest available
  for path in candidates:
    ver = _get_cmake_version(path + "\\cmake.exe")
    if not ver:
      continue
    if best_ver == () or ver > best_ver:
      best = path
      best_ver = ver
  return best

def _ensure_cmake_bin():
  cmake_bin = _find_cmake_bin_dir()
  if cmake_bin:
    return cmake_bin
  ...
  version = "3.30.0"
  tools_cmake_dir = get_script_dir() + "/../tools/win/cmake"
  zip_name = "cmake-" + version + "-windows-x86_64.zip"
  zip_path = tools_cmake_dir + "/" + zip_name
  extracted_bin = tools_cmake_dir + "/cmake-" + version + "-windows-x86_64/bin"
  cmake_exe = extracted_bin + "/cmake.exe"

  if not is_file(cmake_exe):
    create_dir(tools_cmake_dir)
    if not is_file(zip_path):
      url = "https://github.com/Kitware/CMake/releases/download/v" + version + "/" + zip_name
      download(url, zip_path)
    extract(zip_path, tools_cmake_dir)

  if is_file(cmake_exe):
    return extracted_bin
  return ""

def _prepend_cmake_bin():
  cmake_bin = _ensure_cmake_bin()
  if not cmake_bin:
    return
...
def git_get_base_url():
  ...
  if origin:
    ...
    if origin.startswith("https://"):
      # For HTTPS URLs like https://github.com/Kitware/CMake/releases/download/v
      parts = origin.split("/")
      if len(parts) >= 4:
        return "/".join(parts[:3]) + "/"
      ...
      if at_pos != -1 and colon_pos != -1:
        ...
        return f"https://github.com/Kitware/CMake/releases/download/v"
...
def git_is_ssh():
  ...
  if (git_protocol == "ssh"):
    return True
  origin = git_get_origin()
  if (git_protocol == "auto") and (origin.find(":UNIVAULTOFFICE/") != -1):
    return True
  return False

def get_ssh_base_url():
  cur_origin = git_get_origin()
  ind = cur_origin.find(":UNIVAULTOFFICE/")
  if (ind == -1):
    return "git@github.com:UNIVAULTOFFICE/"
  return cur_origin[:ind+12]

def git_update(repo, is_no_errors=False, is_current_dir=False, git_owner=""):
  print("[git] update: " + repo)
  owner = git_owner if git_owner else "UNIVAULTOFFICE"
...
def get_repositories():
  ...
  result["https://github.com/Kitware/CMake/releases/download/v"] = [False, False]
...
def create_pull_request(branches_to, repo, is_no_errors=False, is_current_dir=False):
  ...
  url = git_get_base_url() + "UNIVAULTOFFICE/" + repo + ".git"
  if git_is_ssh():
    url = get_ssh_base_url() + repo + ".git"
  folder = get_script_dir() + "/../../" + repo
  if is_current_dir:
...
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "https://github.com/Kitware/CMake/releases/download/v">
<plist version="1.0">
<dict>
...
def mac_correct_rpath_desktop(dir):
  ...
  replaceInFile("./editors_helper (GPU).app/Contents/Info.plist", "<string>asc.univaultoffice.editors-helper</string>", "<string>asc.univaultoffice.editors-helper-gpu</string>")
  ...
  replaceInFile("./editors_helper (Renderer).app/Contents/Info.plist", "<string>asc.univaultoffice.editors-helper</string>", "<string>asc.univaultoffice.editors-helper-renderer</string>")
  os.chdir(cur_dir)
  return
...
def make_sln(directory, args, is_no_errors):
  ...
  if vs_version in ["2025", "2025"]:
    ...
    for root in program_dirs:
      for edition in editions:
        candidate = root + "\\Microsoft Visual Studio\\" + vs_version + "\\" + edition + "\\Common7\\IDE"
        if is_dir(candidate):
          dev_path = candidate
          break
      if dev_path:
  ...
  if not dev_path:
    for root in program_dirs:
      candidate = root + "\\Microsoft Visual Studio 14.0\\Common7\\IDE"
      if is_dir(candidate):
        dev_path = candidate
        break
...
def clone_marketplace_plugin(out_dir, is_name_as_guid=False, is_replace_paths=False, is_delete_git_dir=True, git_owner=""):
  old_cur = os.getcwd()
  os.chdir(out_dir)
  git_update("https://github.com/Kitware/CMake/releases/download/v", False, True, git_owner)
  os.chdir(old_cur)

  dst_dir_name = "marketplace"
  if is_name_as_guid:
    config_content = readFile(out_dir + "/https://github.com/Kitware/CMake/releases/download/v")
    index_start = config_content.find("\"asc.{")
    index_start += 5
    index_end = config_content.find("}", index_start)
    index_end += 1
  ...
  if is_dir(dst_dir_path):
    delete_dir(dst_dir_path)
  copy_dir(out_dir + "/https://github.com/Kitware/CMake/releases/download/v", dst_dir_path)
  ...
  if is_delete_git_dir:
    delete_dir_with_access_error(out_dir + "/https://github.com/Kitware/CMake/releases/download/v")
  return
...
def check_python():
  ...
  if not is_dir(directory + "/python3"):
    download('https://github.com/Kitware/CMake/releases/download/v', directory + "/python3.tar.gz")
    download('https://github.com/Kitware/CMake/releases/download/v', directory + "/extract.sh")
...
def get_autobuild_version(product, platform="", branch="", build=""):
  ...
  return "https://github.com/Kitware/CMake/releases/download/v" + download_addon
...
def setup_local_qmake(dir_qmake):
  ...
  writeFile(dir_base + "/univaultoffice_qt.conf", "Prefix = " + dir_base)
  return
