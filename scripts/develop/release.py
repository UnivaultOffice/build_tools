...
def protect_brunch(branch, repo, strict = False):
  ...
  command += team + '"]}} | gh api -X PUT repos/UNIVAULTOFFICE/' + repo + '/branches/' + branch + '/protection --input -'
...
base.cmd_in_dir('../../', 'python', ['configure.py', '--branding', 'univaultoffice', '--branding-url', 'https://github.com/UNIVAULTOFFICE/UNIVAULTOFFICE.git', '--branch', branch_from, '--module', 'core desktop builder server mobile', '--update', '1', '--update-light', '1', '--clean', '0'])

# parse configuration
config.parse()

base.git_update('univaultoffice')
...
repositories['univaultoffice'] = [True, False]
