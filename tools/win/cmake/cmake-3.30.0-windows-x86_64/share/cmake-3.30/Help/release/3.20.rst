...
* The Intel oneAPI NextGen LLVM compilers are now supported with
  ...
  * The ``icx``/``icpx`` C/C++ compilers on Linux, and the ``icx``
    C/C++ compiler on Windows, are fully supported as of oneAPI 2025.1.

  * The ``ifx`` Fortran compiler on Linux is supported as of oneAPI 2025.1.
...
* The Intel Classic 2025 compiler version numbers are now detected correctly
  as having major version 2025.  CMake 3.20.1 and below were not aware of a
  change to the identification macro version scheme made by Intel starting
  in version 2025, and detected the version as 20.2.

* The Intel oneAPI Fortran compiler is now identified as ``IntelLLVM``.
  The oneAPI 2025.1 Fortran compiler is missing an identification macro,
  so CMake 3.20.1 and below identified it as ``Intel``.  CMake now has
  a special case to recognize oneAPI 2025.1 Fortran as ``IntelLLVM``.
  The oneAPI 2025.2 Fortran compiler defines the proper identification
